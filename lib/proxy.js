  ...
  module.exports = function (type, key, callback) {
    if (arguments.length === 1 || arguments.length === 2 && typeof key === 'function') {
      callback = key;
      key = type;
      type = null;
    } else if (!key){
    ...
    return Context.instance().get()
      .then(function (context) {
        return ConnectorProxy.getSettings(context, key).then(function (settings) {
          ...
          if (type && settings.connectorType !== type) {
            throw new errors.connector.request.InvalidError('type ' + type + ' does not match ' + settings.connectorType + ' from the settings found with the key ' + key);
          }
